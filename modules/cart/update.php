...
function logError($msg) {
    error_log(date('Y-m-d H:i:s') . " Cart Update: " . $msg);
}

try {
    logError("Starting update request");
    ...
    logError("Cart ID: $cart_id, Quantity: $quantity");
    
    // Validate
    if ($cart_id <= 0 || $quantity <= 0) {
        logError("Invalid data");
        outputJson(['success' => false, 'message' => 'Invalid data']);
    }
    ...
    if ($stmt->rowCount() === 0) {
        logError("Cart item not found");
        outputJson(['success' => false, 'message' => 'Item not found']);
    }
    ...
    if ($item['stock_quantity'] < $quantity) {
        logError("Not enough stock");
        outputJson(['success' => false, 'message' => 'Not enough stock']);
    }
    
    // Update
    $stmt = $db->prepare("UPDATE cart SET quantity = ? WHERE id = ? AND user_id = ?");
    if ($stmt->execute([$quantity, $cart_id, $_SESSION['user_id']])) {
        logError("Update successful");
        outputJson(['success' => true, 'message' => 'Updated']);
    } else {
        logError("Update failed");
        outputJson(['success' => false, 'message' => 'Update failed']);
    }
